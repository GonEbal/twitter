document.addEventListener('DOMContentLoaded', function() {
    ...
    fetch(`${window.location.pathname}/tweets`)
		.then(response => response.json())
	    .then(data => {
    ...
    function define_user(info) {
        document.querySelector('.profile-username-name').innerHTML = info.info.username
        document.querySelector('.f_count').innerHTML = info.info.followers
        document.querySelector('.Follow').setAttribute("onclick", `follow(${JSON.stringify(info.info.username)})`)
        document.querySelector('.follow_text').innerHTML = info.button;
        document.querySelector('.fl_count').innerHTML = info.following
        document.querySelector('.pt_count').innerHTML = info.posts_count
	...
	function add_post(contents) {
                ...
                // Add post to DOM
                document.querySelector('.lenta').append(post);
...
function follow(username) {
        ...
        .then(response => response.json())
        .then(data => {
            console.log(data.info.followers)
            document.querySelector('.f_count').innerHTML = data.info.followers;
            document.querySelector('.fl_count').innerHTML = data.following
            document.querySelector('.follow_text').innerHTML = data.button;
            //document.getElementById('followunfollow').setAttribute("class", "Unfollow");
